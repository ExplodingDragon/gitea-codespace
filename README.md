...
make test-e2e-manager-required
```

`make test` 运行不依赖真实 Incus 的 Go 测试和脚本语法检查。`make test-e2e-auto` 只在本机 Incus client 可用且 `incus info` 成功时运行真实 E2E，否则输出跳过原因；`*-required` 入口用于 CI 或部署验收，Incus 条件不满足时直接失败。这样普通开发不被宿主环境阻塞，明确要求真实后端验收时也能得到硬错误。
