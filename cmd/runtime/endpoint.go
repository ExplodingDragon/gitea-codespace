...
type setEndpointFunc func(port uint16, label string, public bool) error
...
func newEndpointCommandWithRun(setEndpoint setEndpointFunc, deleteEndpoint deleteEndpointFunc) *cobra.Command {
	...
	listCommand := &cobra.Command{
		...
		RunE: func(command *cobra.Command, _ []string) error {
			...
			for _, endpoint := range endpoints {
				...
				if _, err := fmt.Fprintf(writer, "%d\t%s\t%s\n", endpoint.UpstreamPort, visibility, endpoint.Label); err != nil {
					return err
				}
			}
			return writer.Flush()
	...
	setCommand := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			return setEndpoint(uint16(port), label, public)
		},
	}
