...
这个目录提供一套完整的 `init`、`start`、`resume` 自定义脚本，用来说明 Codespace Manager 如何通过通用脚本契约接入 devcontainer。设计重点是：Manager 管理 Incus 实例、通用输入、共享环境、结果文件、workspace 和实际 Endpoint 端口；Docker、devcontainer CLI、容器身份和容器内服务启动都由脚本负责。Gateway 的 shell、exec、SFTP 和 Web 终端由 Manager 通过 Incus API 进入实例 workspace，不要求 devcontainer 内运行 sshd。
...
scripts:
  ...
  resume: /path/to/codespace/examples/devcontainer/resume.sh
```
...
- `start.sh prepare` 克隆或复用 workspace，按 create payload 锁定 commit，然后执行 `devcontainer up --workspace-folder`。
- `resume.sh prepare` 保留当前 workspace HEAD，只恢复 Git 凭据并启动已有 devcontainer；缺失容器时重新执行 `devcontainer up`。
- `activate` 启动已有 devcontainer，并把容器 ID 继续写入共享环境，供后续 resume 复用。
- 普通 Endpoint 由容器或用户进程先暴露到 Incus 实例内的实际端口，再调用 Runtime Endpoint API。Manager 不读取容器端口或容器 ID。

实现验收点：

- [x] 三个脚本可以作为完整自定义套件显式配置，Manager 使用与内置脚本相同的 init、prepare、activate、共享环境和结果文件契约调用。
...
- [x] resume 不执行 checkout，不依赖 repository payload，恢复已有 workspace、Git 凭据和 devcontainer。
